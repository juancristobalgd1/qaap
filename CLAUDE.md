...
- [-] **monaco** (`monaco-quick-input-{layout,service}.ts`, `monaco-frontend-module.ts`). **Accepted as permanent seam.** The factory `MonacoQuickInputLayout` is a textbook upstream-style DI seam consumed by `qaap-adapters`; reverting and subclassing `MonacoQuickInputImplementation` would require duplicating `@postConstruct init()` (~30 lines copy-paste). Net result is worse coupling.
...
- [x] **core** (residuals after Tier 1–3 cleanups). Mostly already-allowlisted small seams **justified by qaap consumers** (e.g. `WorkbenchTopBarFactory` → `qaap-work-hub`, `ElectronMainApplication.resolveApplicationIconPath` → `qaap-product`). Real outstanding work:
