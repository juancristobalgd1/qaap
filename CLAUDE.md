...
- [x] **Non-AI infrastructure sweep (July 2026, wave 2):** debug, filesystem, preferences, terminal, task, scm(-extra), notebook, output, process — 77 baseline files triaged, ~64 re-adopted from upstream in 7 bisectable batches. Functional fixes recovered: task-server `onDidStartTaskProcess` never fired (dropped `task` arg), terminal buffer flush on process close, preferences storage dispose() chain, parcel-watcher ignorePatterns + ENOENT retry, terminal paste/copy commands (prefs existed but were orphaned), upstream debug-session DI refactor (incl. the plugin-ext factory adaptation). Remaining per-package baseline entries:
    ...
    - `filesystem/.../file-tree-decorator-adapter.ts` — DO NOT blind-revert: upstream appears to have rolled back its own on-demand rewrite that we adopted in May (commit `47e101d3e`); verify against real Theia GitHub history first (re-reverting would reintroduce the maxEventSize decoration-drop bug).
...
- [x] **Wave 3 (July 2026):** vsx-registry, dev-container/remote/remote-wsl, and 14 misc packages (~69 files triaged). Adopted: upstream's unified Extensions view (ExtensionsSourceContribution + ExtensionCard/TypeBadge + refresh command — whole feature had never been merged; mobile CSS class names preserved), keymaps live key/chord capture in EditKeybindingDialog, monaco registerEditorContainer-on-start, workspace `getRootPrefixedPath()` (additive; its ai-* consumers are still pending sync), remote auto-shutdown service, docker-free style reversions everywhere. New `ALLOWED` seams: file-search spec `.worktrees` exclude (agent runtime creates real worktrees), and the fork's Buffer/ReadableStream extension of `RemoteConnection.copy()` (remote-types.ts + remote-wsl-connection.ts; SSH impl still string-only — follow-up ticket).
    - **Deferred (dedicated session): dev-container "Attach to Container" cluster (~16 coupled files).** Upstream extracted `RemoteDockerContainerConnection` to its own file and added workspace-candidate scanning, `devcontainer.attach.applyFoundConfig` preference, `--attach-container` CLI startup flow, and docker progress dedup. The fork has a deliberately simplified attach implementation plus a different variable-resolver design (`DockerContainerService.container` + LazyServiceIdentifier instead of upstream's `VariableContext` param). All coupled through the RPC contract in `electron-common/remote-container-connection-provider.ts` — re-adopt as one unit, decide the resolver pattern first, preserve the fork's Buffer/stream `copy()` branch, and migrate the fork-only `remote-container-connection-provider.spec.ts` (remoteEnv test) instead of discarding it.
