...
import { classifyTranscriptToolActivityKind, excerptTranscriptThought, extractInlineDiffPreview, extractTranscriptDiffCard, extractTranscriptMcpServerLabel, isTranscriptMcpTool, isTranscriptThoughtExcerptTruncated, isTranscriptTodoTool, parseTranscriptTodoChecklist, resolveTranscriptActivityStats, resolveTranscriptThinkingContent, resolveTranscriptToolPillDescriptors, resolveTranscriptToolRowParts, shouldOpenTranscriptToolDetails } from './qaap-agent-transcript-segments';
...
describe('classifyTranscriptToolActivityKind', () => {
    it('maps common tool names to activity buckets', () => {
        ...
        expect(classifyTranscriptToolActivityKind('Update')).to.equal('editing');
        expect(classifyTranscriptToolActivityKind('apply_patch')).to.equal('editing');
