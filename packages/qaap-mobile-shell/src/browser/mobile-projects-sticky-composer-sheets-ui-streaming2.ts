...
export function openStickyComposerApprovalPolicySheetExtracted(ctx: any, project: MobileProjectEntry,
        ...
        ctx.openApprovalPolicySheet({
            ...
            toolRules: reconcileAgentToolApprovalRules(
                reconcileAgentApprovalPolicyId(ctx.host.stickyComposerApprovalPolicyId, cwd),
                cwd,
                ctx.host.stickyComposerToolApprovalRules,
            ),
            isOpen: () => ctx.host.stickyComposerApprovalSheet !== undefined,
            onSelect: policyId => {
                ctx.host.stickyComposerApprovalPolicyId = policyId;
                ctx.host.stickyComposerToolApprovalRules = reconcileAgentToolApprovalRules(
                    policyId,
                    cwd,
                    ctx.host.stickyComposerToolApprovalRules,
                );
                if (cwd) {
                    ...
                    writeStoredAgentToolApprovalRules(cwd, ctx.host.stickyComposerToolApprovalRules);
                }
                ctx.closeAllComposerSheets();
                ctx.host.stickyComposerRenderUi.renderStickyComposer();
            },
            onToolRulesChange: rules => {
                ctx.host.stickyComposerToolApprovalRules = rules;
                if (cwd) {
