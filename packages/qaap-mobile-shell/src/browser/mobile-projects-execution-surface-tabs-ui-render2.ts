...
export function showOnlyExecutionSurfaceTabExtracted(ctx: any, tab: TranscriptTab): void {
        ...
        if (ctx.host.transcriptPlanHost) {
            ctx.host.transcriptPlanHost.hidden = tab !== 'plan';
        }
...
export function syncSurfaceHostsFromContainerExtracted(ctx: any, container: HTMLElement): void {
        ctx.host.transcriptPlanHost = ctx.directChildWithClass(container, 'theia-mobile-transcript-plan') ?? ctx.host.transcriptPlanHost;
