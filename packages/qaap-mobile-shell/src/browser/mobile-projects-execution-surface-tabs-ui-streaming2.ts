...
export function resolveTerminalAgentTuiActiveAgentIdExtracted(ctx: any, project?: MobileProjectEntry): string | undefined {
        ...
        if (resolvedProject) {
            return ctx.host.stickyComposerAgentsUi.resolveStickyComposerPinnedAgentId(resolvedProject);
        }
        const fromSummary = ctx.host.transcriptOpenSummary?.agentId?.trim();
        if (fromSummary && fromSummary !== 'task') {
            return fromSummary;
        }
        return ctx.host.stickyComposerPinnedAgentId;
}
