...
export function mountProjectDetailSurfaceTabExtracted(ctx: any, project: MobileProjectEntry,
        ...
        switch (tab) {
            case 'plan':
                ctx.renderPlanTab(ctx.host.projectDetailSurfaceTargets?.planHost, undefined);
                break;
            case 'review':
...
export function renderPlanTabExtracted(ctx: any, host: HTMLElement | undefined, conv: QaapAgentConversationDTO | undefined): void {
        if (!host) {
            return;
        }
        host.replaceChildren();
        ...
        const segments = ctx.latestAgentSegments(conv);
        ...
        const items = ctx.host.transcriptMessagesUi.resolveTranscriptActivityItems(segments);
        if (items.length === 0) {
            const note = document.createElement('div');
            note.className = 'theia-mobile-transcript-plan-note';
            note.textContent = nls.localize(
        ...
        const timeline = ctx.host.transcriptMessagesUi.createTranscriptActivityTimeline(segments, {
            variant: 'plan',
            maxVisibleItems: 0,
        });
        if (timeline) {
