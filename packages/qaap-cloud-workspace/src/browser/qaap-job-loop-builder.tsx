...
export function QaapJobLoopBuilder(props: QaapJobLoopBuilderProps): React.ReactNode {
    ...
    return <form className='qaap-job-loop-builder' onSubmit={event => void submit(event)} noValidate>
        ...
        <fieldset><legend>{nls.localize('qaap/jobLoops/builder/condition', 'Stop condition')}</legend>
            ...
            <Field label={nls.localize('qaap/jobLoops/builder/sourcePointer', 'Source JSON Pointer')} error={show('conditionPointer')}><input value={props.draft.conditionPointer} onChange={e => update({ conditionPointer: e.currentTarget.value })} /></Field>
            ...
            {!['truthy', 'falsy'].includes(props.draft.conditionOperator) && <Field label={nls.localize('qaap/jobLoops/builder/expected', 'Expected JSON value')} error={show('conditionExpected')}><input value={props.draft.conditionExpected} onChange={e => update({ conditionExpected: e.currentTarget.value })} /></Field>}
        </fieldset>
