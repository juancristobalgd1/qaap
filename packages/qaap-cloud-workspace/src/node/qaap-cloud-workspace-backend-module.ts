...
export default new ContainerModule((bind, _unbind, _isBound, rebind, _unbindAsync, onActivation) => {
    ...
    bind(QaapDockerDevPreviewUpstreamTunnel).toSelf().inSingletonScope();
    bind(QaapDevPreviewUpstreamTunnel).toService(QaapDockerDevPreviewUpstreamTunnel);
    ...
    // provider on dispose — routing it through the tenant guard made the boot-time
    // `watch` of backend-settings.json throw an uncaught "Forbidden workspace path".
    bindQaapTenantDiskFileSystemProvider(bind, rebind);
