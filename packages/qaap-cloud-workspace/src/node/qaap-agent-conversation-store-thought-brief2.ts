...
export function settleChatTurnRunExtracted(ctx: QaapAgentConversationStoreContext, task: QaapAgentTask, outcome: QaapWorkflowNodeOutcome = resolveChatTurnOutcome(task.state)): void {
...
export function findLiveChatTurnRunExtracted(ctx: QaapAgentConversationStoreContext, conv: QaapAgentConversation, rootUserMessageId: string): QaapPersistedWorkflowRun | undefined {
...
export function countDurableLoopSpawnsExtracted(ctx: QaapAgentConversationStoreContext, conv: QaapAgentConversation,
        rootUserMessageId: string,
        record: QaapPersistedWorkflowRun | undefined, ): number {
        return countDurableLoopSpawnsHelper(conv, rootUserMessageId, record);
}

export async function maybeRetryTurnWithFallbackModelViaGraphExtracted(ctx: QaapAgentConversationStoreContext, conversationId: string,
...
export async function reapOrphanedChatTurnRunsExtracted(ctx: QaapAgentConversationStoreContext): Promise<void> {
        const runs = ctx.workflowRuns;
        if (!runs) {
            return;
        }
...
export function interruptStreamingTurnForRestartExtracted(ctx: QaapAgentConversationStoreContext, conversationId: string, nowMs: number): boolean {
